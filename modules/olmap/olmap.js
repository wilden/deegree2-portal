...
/*----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
/**
 * encapsulating openlayers map for enabling communication with iGeoPortal
 * controller
 */
function OLMap(mapModel) {
	...
	/**
	 * implements mandatory paint method; will be invoke first time a module
	 * will be displayed
	 */
	...
	/**
	 * implements mandatory repaint method; will be invoked every time
	 * repaint-method of the controller will be invoked
	 */
	function repaint() {
...
//		this.map.addLayer( wfslayer );
	...
	function pushLayers() {
		...
		var dummy  =  new OpenLayers.Layer.Vector("_DUMMY_BASE_", { isBaseLayer: true });		
		...
		for ( var j = layerGroups.length-1; j >= 0 ; j--) {
			...
			for ( var i = 0; i < vLayers.length; i++) {
				if ( i > 0 && vLayers[i].isTiled() != vLayers[i-1].isTiled() ) {					
					...
					var layer = new OpenLayers.Layer.WMS( layerGroups[j].getServiceName() + j + " " + i, 
							layerGroups[j].getServiceURL(), 
							{ layers : l, transparent: true, format: 'image/png', sessionID: sid },
			...
			if ( l.length > 1 ) {
				l = l.substring( 0, l.length-1 );			
				var layer = new OpenLayers.Layer.WMS( layerGroups[j].getServiceName() + j, 
						layerGroups[j].getServiceURL(), 
						{ layers : l, transparent: true, format: 'image/png', sessionID: sid},
	...
	/**
	 * initializes openlayers map by adding visible layers and zooming to
	 * current bbox
	 * 
	 * @return
	 */
	...
	function mapEvent(e) {
		...
		var event = new Event(this, "BBOX", new Envelope(bb[0], bb[1], bb[2],
				bb[3]));
